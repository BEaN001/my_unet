...
class UpConvBlock(nn.Module):
    def __init__(self, in_chans, bridge_chans_list, out_chans):
        ...
        self.conv_block = BasicBlock(in_channels=out_chans + sum(bridge_chans_list), out_channels=out_chans, stride=1)
...
class ResNet_UNetpp(nn.Module):
    ...
    def __init__(self, n_classes=2, backbone=resnet34(in_chans=3)):
        ...
        feat_chans中可以有相同的数字
        '''
        feat_chans = [backbone.block.expansion * i for i in [64, 128, 256]]
    ...
    def forward(self, x):
        ...
        替换为backbone的输出
        '''
        # print("resnet_unetppL2!")
        x00, x10, x20 = self.encoder(x)
        # x00 = self.conv_x00(x)
...
if __name__ == "__main__":
    print(30*'*')
    print('resnet34_unet_pp_L2')
    x = torch.randn((2, 3, 112, 112), dtype=torch.float32)
    model = ResNet_UNetpp(n_classes=3, backbone=resnet34(in_chans=3))
    y1, y2 = model(x)
    print(30 * '*')
    print('resnet50_unet_pp_L2')
    x = torch.randn((2, 3, 112, 112), dtype=torch.float32).cuda()
    model = ResNet_UNetpp(n_classes=3, backbone=resnet50(in_chans=3)).cuda()
    y1_, y2_ = model(x)

    # from torchsummary import summary
    # summary_vision = summary(model, (3, 572, 572))